...
1. **The LLM** - this would be to evaluate how competent our model is, without any bells and whistles. Great to do before you decide what model to use or upgrade to a new model.
...
This is important to test both for your model based on the hardware you use, but also on the backend system as a whole, as when we keep adding more complex functionality it will slow down how fast the model can respond, sometimes causing it to be an unviable option for our usecase.
